...
%token CASSIGN_ADD CASSIGN_SUB CASSIGN_MULT CASSIGN_DIV COMP_EQU COMP_NEQU COMP_LESS COMP_LTE COMP_GTR COMP_GTE BOOL_AND BOOL_OR COMMAND_PRINT COMMAND_IF COMMAND_ELSE COMMAND_WHILE COMMAND_FOR COMMAND_BREAK COMMAND_CONTINUE COMMAND_RANDOM
...
program:      statement_list {
                ...
                TC_Array TC_Array;  // Intermediate code container

                // Traverse the AST, filling TC_Array with code
                $1->CompileTubeIC(symbol_table, TC_Array);

                // Write Intermediate Code to the specified output file!
                std::ofstream out_file(out_filename.c_str());
                TC_Array.PrintIC(out_file);
              }
