...
%token CASSIGN_ADD CASSIGN_SUB CASSIGN_MULT CASSIGN_DIV COMP_EQU COMP_NEQU COMP_LESS COMP_LTE COMP_GTR COMP_GTE BOOL_AND BOOL_OR COMMAND_PRINT COMMAND_IF COMMAND_ELSE COMMAND_WHILE COMMAND_FOR COMMAND_BREAK COMMAND_CONTINUE COMMAND_RANDOM FUNCTION_DEFINE FUNCTION_RETURN
...
%type <ast_node> expression declare_assign statement statement_list var_usage array_index lhs_ok command argument_list argument code_block for_init opt_expr if_start while_start for_start flow_command function function_start
%type <value> type_any
...
statement:   var_declare ';'    {  $$ = NULL; /* Only update symbol table */ }
	 ...
	 |   command ';'        {  $$ = $1;  }
         ...
         |   FUNCTION_RETURN expression ';' { if (table_entry == NULL) exit(1); 
                                              $$ = new ASTNode_Function_Return(table_entry, $2); }
...
expression:  expression '+' expression {
  ...
  |   ID '(' ')' { tableEntry * cur_entry = symbol_table.Lookup($1);
               if (cur_entry == NULL) {
     std::string err_string = "unknown variable '";
     err_string += $1;
                 err_string += "'";
     yyerror(err_string);
         ...
         $$ = new ASTNode_Function_Call(cur_entry);
               $$->SetLineNum(line_num);
     }
  |   ID '(' argument_list ')' { tableEntry * cur_entry = symbol_table.Lookup($1);
               if (cur_entry == NULL) {
     std::string err_string = "unknown variable '";
     err_string += $1;
                 err_string += "'";
     yyerror(err_string);
               ...
               ASTNode_Function_Call * node = new ASTNode_Function_Call(cur_entry);
               node->TransferChildren($3);
               delete $3;
               std::vector<tableEntry*> check = cur_entry->GetArgs();
               if ( (int)check.size() != node->GetNumChildren())
               {
                exit(1);
               }
               for (unsigned int i = 0; i < check.size() ; i++)
               {
                if (check[i]->GetType() != node->GetChild(i)->GetType())
                {
                  exit(1);
                }
               }
...
function:  function_start function_end '{' statement_list '}' {
            table_entry->SetAST($4);
            $$ = new ASTNode_Function(table_entry, $4);
            symbol_table.DecScope();
            table_entry = NULL;
          }
        |  function_start function_end  FUNCTION_RETURN expression ';' {
            $$ = new ASTNode_Function(table_entry, new ASTNode_Function_Return(table_entry, $4));
            symbol_table.DecScope();
            table_entry = NULL;
          }

function_start:  FUNCTION_DEFINE type_any ID '(' {
                  table_entry = symbol_table.AddFunction($2, $3);
                  if (symbol_table.GetCurScope() != 0) { yyerror("not in global scope"); }

                  symbol_table.IncScope();

                }

function_end:  function_args ')' {  }
            |  ')' {  }

function_args: var_declare { table_entry->AddArg($1); }
            |  function_args ',' var_declare { table_entry->AddArg($3); }
