...
private:
  tableEntry * function_entry;
public:
  ASTNode_Function(tableEntry * table_entry, ASTNode * in);
  virtual ~ASTNode_Function() { ; }

  tableEntry * CompileTubeIC(symbolTable & table, IC_Array & ica);
  virtual std::string GetName() {
    std::string out_string = "ASTNode_Function";
    return out_string;
  }
};

class ASTNode_Function_Return : public ASTNode {
private:
  tableEntry * function_entry;
public:
  ASTNode_Function_Return(tableEntry * table_entry, ASTNode * in);
  virtual ~ASTNode_Function_Return() { ; }

  tableEntry * CompileTubeIC(symbolTable & table, IC_Array & ica);
  virtual std::string GetName() {
    std::string out_string = "ASTNode_Function_Return";
    return out_string;
  }
};
...
private:
  tableEntry * function_entry;
public:
  ASTNode_Function_Call(tableEntry * in_entry);

  tableEntry * GetFunctionEntry() { return function_entry; }
  tableEntry * CompileTubeIC(symbolTable & table, IC_Array & ica);

  virtual std::string GetName() {
    std::string out_string = "ASTNode_Function_Call (";
    out_string += function_entry->GetName();
    out_string += ")";
    return out_string;
  }
};

class ASTNode_FunctionDefine : public ASTNode {
private:
  tableEntry * function_entry;
public:
  ASTNode_FunctionDefine(tableEntry * table_entry, ASTNode * in);
  virtual ~ASTNode_FunctionDefine() { ; }

  tableEntry * CompileTubeIC(symbolTable & table, IC_Array & ica);
  virtual std::string GetName() {
    std::string out_string = "ASTNode_FunctionDefine";
    return out_string;
  }
};
