...
protected:
  ...
  ASTNode * ast_node;
  std::vector<tableEntry*> args_type;
  ...
  tableEntry(int in_type, const std::string in_name, ASTNode * ast)
    : type_id(in_type)
    , name(in_name)
    , is_temp(false)
    , var_id(-1)
...
public:
  ...
  void SetAST(ASTNode * ast) { ast_node = ast; }
  ASTNode * GetAST() const { return ast_node; }
  ...
  void AddArg(tableEntry* type) { args_type.push_back(type); }
  std::vector<tableEntry*> GetArgs() { return args_type; }
};
...
public:
  ...
  tableEntry * AddFunction(int in_type, std::string in_name) {
