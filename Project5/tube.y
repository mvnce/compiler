...
%token CTRL_IF CTRL_ELSE CTRL_WHILE CTRL_BREAK CTRL_CONTINUE ARRAY

%token <lexeme> CHAR VALUE TYPE ID COMMAND_PRINT COMMAND_RANDOM STRING
%left BOOL_OR
%left BOOL_AND
...
| ID '[' expression ']' {
            if ($3->GetType() != 1)
            {
              std::string err_string = "array indices must be of type val";
              yyerror(err_string);
              exit(1);
            }
            tableEntry * cur_entry = symbol_table.Lookup($1);
            ...
            //cur_entry->SetIndex(atoi($3));
            if (cur_entry->GetType() == 3 || cur_entry->GetType() == 4)
            {
              $$ = new ASTNode_arr_Variable(cur_entry,$3);
            }
            else
            {
              std::string err_string = "cannot index into a non-array type '";
              err_string += $1;
              err_string += "'";
              yyerror(err_string);
              exit(1);
...
var_declare:  TYPE ID {
  ...
  |     ARRAY '(' TYPE ')' ID {
        ...
        int type_id = 0;
        if (type_name == "val") type_id = Type::VAL_ARRAY;
        ...
        tableEntry * cur_entry = symbol_table.Insert(type_id, $5);
...
expression:  expression '+' expression { 
  ...
  |    STRING {
               $$ = new ASTNode_Char(Type::STRING, $1);
             }
  |    var_usage { $$ = $1; }
